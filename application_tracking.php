...
$applications = $db->fetchAll("
    SELECT p.*, a.titre, a.entreprise, a.company_name, a.salary_min, a.salary_max,
           d.nom as domaine_nom, v.nom as ville_nom, c.nom as contrat_nom,
           CASE 
               WHEN p.status = 'applied' THEN 1
               WHEN p.status = 'viewed' THEN 2
               WHEN p.status = 'shortlisted' THEN 3
               WHEN p.status = 'interviewed' THEN 4
               WHEN p.status = 'hired' THEN 5
               WHEN p.status = 'rejected' THEN 0
               ELSE 0
           END as status_order
    FROM postulation p
...
foreach ($applications as $app) {
    switch ($app['status']) {
        case 'applied': $applied_count++; break;
        case 'viewed': $viewed_count++; break;
        case 'shortlisted': $shortlisted_count++; break;
        case 'interviewed': $interviewed_count++; break;
        case 'hired': $hired_count++; break;
        case 'rejected': $rejected_count++; break;
    }
}

// Calculate success rate
$success_rate = $total_applications > 0 ? round((($viewed_count + $shortlisted_count + $interviewed_count + $hired_count) / $total_applications) * 100, 1) : 0;

// Get recent activity
$recent_activity = $db->fetchAll("
    SELECT p.applied_at, p.status, a.titre, a.entreprise, 
           CASE 
               WHEN p.status = 'applied' THEN 'Candidature soumise'
               WHEN p.status = 'viewed' THEN 'Candidature consultée'
               WHEN p.status = 'shortlisted' THEN 'Candidature présélectionnée'
               WHEN p.status = 'interviewed' THEN 'Entretien programmé'
               WHEN p.status = 'hired' THEN 'Embauche confirmée'
               WHEN p.status = 'rejected' THEN 'Candidature refusée'
               ELSE 'Statut inconnu'
           END as status_text
    FROM postulation p
    JOIN annonces a ON p.annonce_id = a.id
...
$trends = $db->fetchAll("
    SELECT DATE(p.applied_at) as date, COUNT(*) as count
    FROM postulation p
    WHERE p.user_id = ? AND p.applied_at >= DATE_SUB(CURDATE(), INTERVAL 30 DAY)
    GROUP BY DATE(p.applied_at)
    ORDER BY date DESC
", [$user_id]);
            ...
            <div class="row">
                <div class="col-12">
                    ...
                    <div class="professional-section">
                        <h3><i class="fas fa-tasks"></i>Suivi des Candidatures</h3>
                        <div class="row">
                            <div class="col-md-8">
                                <div class="enhanced-card">
                                    <div class="enhanced-card-header">
                                        <h4 class="enhanced-card-title">Mes Candidatures</h4>
                                        <div class="enhanced-card-actions">
                                            <button class="btn-action btn-action-primary" onclick="refreshApplications()">
                                                <i class="fas fa-sync-alt"></i>Actualiser
                                            </button>
                                            <button class="btn-action btn-action-success" onclick="exportApplications()">
                                                <i class="fas fa-download"></i>Exporter
                                            </button>
                                        </div>
                                    </div>
                                    <div class="d-flex justify-content-between align-items-center mb-3">
                                        <span class="text-muted"><?= $total_applications ?> candidatures au total</span>
                    ...
                    <div class="professional-section">
                        <h3><i class="fas fa-chart-bar"></i>Statistiques des Candidatures</h3>
                        <div class="row">
                            <div class="col-md-2">
                            ...
                            <div class="col-md-2">
                                <div class="stats-card">
                                    ...
                                    <div class="stats-number"><?= $applied_count ?></div>
                                    <div class="stats-label">Postulées</div>
                                </div>
                            </div>
                            <div class="col-md-2">
                                <div class="stats-card">
                                    ...
                                    <div class="stats-number"><?= $viewed_count ?></div>
                            ...
                            <div class="col-md-2">
                                <div class="stats-card">
                                    ...
                                    <div class="stats-number"><?= $shortlisted_count ?></div>
                                    <div class="stats-label">Sélectionnées</div>
                                </div>
                            </div>
                            <div class="col-md-2">
                                <div class="stats-card">
                                    ...
                                    <div class="stats-number"><?= $interviewed_count ?></div>
                                    <div class="stats-label">Entretiens</div>
                                </div>
                            </div>
                            <div class="col-md-2">
                                <div class="stats-card">
                                    ...
                                    <div class="stats-number"><?= $hired_count ?></div>
                                    <div class="stats-label">Embauchées</div>
                                </div>
                            </div>
                        </div>
                    </div>

                    <!-- Success Rate and Trends Section -->
                    <div class="professional-section">
                        <h3><i class="fas fa-chart-line"></i>Performance et Tendances</h3>
                        <div class="row">
                            <div class="col-md-4">
                                <div class="enhanced-card">
                                    <div class="enhanced-card-header">
                                        <h4 class="enhanced-card-title">Taux de Réussite</h4>
                                    </div>
                                    <div class="text-center">
                                        <div class="display-4 text-primary mb-2"><?= $success_rate ?>%</div>
                                        <p class="text-muted">Candidatures avec progression</p>
                                        <div class="progress mb-3" style="height: 10px;">
                                            <div class="progress-bar bg-success" style="width: <?= $success_rate ?>%"></div>
                                        </div>
                                        <small class="text-muted">
                                            <?= $viewed_count + $shortlisted_count + $interviewed_count + $hired_count ?> sur <?= $total_applications ?> candidatures
                                        </small>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-8">
                                <div class="enhanced-card">
                                    <div class="enhanced-card-header">
                                        <h4 class="enhanced-card-title">Activité Récente (30 derniers jours)</h4>
                                    </div>
                                    <?php if (!empty($trends)): ?>
                                        <div class="row">
                                            <?php foreach(array_slice($trends, 0, 7) as $trend): ?>
                                                <div class="col-md-2 text-center">
                                                    <div class="border rounded p-2">
                                                        <div class="h5 text-primary mb-1"><?= $trend['count'] ?></div>
                                                        <small class="text-muted"><?= date('d/m', strtotime($trend['date'])) ?></small>
                                                    </div>
                                                </div>
                                            <?php endforeach; ?>
                                        </div>
                    ...
                    <ul class="nav nav-tabs mb-4" id="statusTabs" role="tablist">
                        <li class="nav-item" role="presentation">
                            <a class="nav-link <?= $status_filter === 'all' ? 'active' : '' ?>" 
                               href="?status=all">Toutes (<?= $total_applications ?>)</a>
                        </li>
                        <li class="nav-item" role="presentation">
                            <a class="nav-link <?= $status_filter === 'applied' ? 'active' : '' ?>" 
                               href="?status=applied">Postulées (<?= $applied_count ?>)</a>
                        </li>
                        <li class="nav-item" role="presentation">
                            <a class="nav-link <?= $status_filter === 'viewed' ? 'active' : '' ?>" 
                               href="?status=viewed">Vues (<?= $viewed_count ?>)</a>
                        </li>
                        <li class="nav-item" role="presentation">
                            <a class="nav-link <?= $status_filter === 'shortlisted' ? 'active' : '' ?>" 
                               href="?status=shortlisted">Sélectionnées (<?= $shortlisted_count ?>)</a>
                        </li>
                        <li class="nav-item" role="presentation">
                            <a class="nav-link <?= $status_filter === 'interviewed' ? 'active' : '' ?>" 
                               href="?status=interviewed">Entretiens (<?= $interviewed_count ?>)</a>
                        </li>
                        <li class="nav-item" role="presentation">
                            <a class="nav-link <?= $status_filter === 'hired' ? 'active' : '' ?>" 
                               href="?status=hired">Embauchées (<?= $hired_count ?>)</a>
                        </li>
                        <li class="nav-item" role="presentation">
                            <a class="nav-link <?= $status_filter === 'rejected' ? 'active' : '' ?>" 
                               href="?status=rejected">Refusées (<?= $rejected_count ?>)</a>
                        </li>
                    </ul>
                    ...
                    <div class="professional-section">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="enhanced-card">
                                    <div class="enhanced-card-header">
                                        <h4 class="enhanced-card-title">Dernières Mises à Jour</h4>
                                        <div class="enhanced-card-actions">
                                        ...
                                        <div class="timeline">
                                            <?php foreach($recent_activity as $activity): ?>
                                                <div class="timeline-item">
                                                    <div class="timeline-marker bg-<?= getStatusBadgeClass($activity['status']) ?>">
                                                    ...
                                                    <div class="timeline-content">
                                                        ...
                                                        <div class="d-flex justify-content-between align-items-center">
                                                            ...
                                                            <small class="text-muted">
                                                                ...
                                                                <?= date('d/m/Y H:i', strtotime($activity['applied_at'])) ?>
                                                            </small>
                                                        </div>
                                                    </div>
                                                </div>
                                        ...
                                        <div class="card-body">
                                            <div class="d-flex justify-content-between align-items-start mb-3">
                                                <h5 class="card-title mb-0">
                                                    <a href="enhanced_job_details.php?id=<?= $app['annonce_id'] ?>" 
                                                       class="text-decoration-none">
                                                        <?= htmlspecialchars($app['titre']) ?>
                                                    </a>
                                                </h5>
                                            ...
                                            <p class="text-muted mb-2">
                                                ...
                                                <?= htmlspecialchars($app['company_name'] ?: $app['entreprise']) ?>
                                            </p>
                                            
                                            <p class="text-muted mb-2">
                                                <i class="fas fa-map-marker-alt me-2"></i>
                                                <?= htmlspecialchars($app['ville_nom']) ?>
                                            </p>
                                            ...
                                            <p class="text-muted mb-3">
                                                ...
                                                Postulé le <?= date('d/m/Y', strtotime($app['applied_at'])) ?>
                                            </p>
                                            
                                            <?php if ($app['employer_notes']): ?>
                                                <div class="alert alert-info">
                                                    <strong>Note de l'employeur:</strong><br>
                                                    <?= htmlspecialchars($app['employer_notes']) ?>
                                                </div>
                                            <?php endif; ?>
                                            
                                            <?php if ($app['interview_date']): ?>
                                                <div class="alert alert-warning">
                                                    <strong>Entretien prévu:</strong><br>
                                                    <?= date('d/m/Y H:i', strtotime($app['interview_date'])) ?>
                                            ...
                                            <div class="d-flex justify-content-between align-items-center">
                                                ...
                                                <?php if ($app['status'] === 'hired' && $app['salary_offered']): ?>
                                                    <span class="text-success">
                                                        <i class="fas fa-check-circle me-1"></i>
                                                        Salaire proposé: <?= number_format($app['salary_offered'], 0, ',', ' ') ?> MAD
                                                    </span>
                                                <?php endif; ?>
                                            </div>
                                        </div>
...
function exportApplications() {
    ...
    let csv = 'Titre,Entreprise,Ville,Statut,Date de candidature\n';
    
    applications.forEach(app => {
        csv += `"${app.titre}","${app.company_name || app.entreprise}","${app.ville_nom}","${getStatusLabel(app.status)}","${app.applied_at}"\n`;
    });
    ...
    a.href = url;
    a.download = 'mes_candidatures.csv';
    a.click();
...
function getStatusBadgeClass($status) {
    switch ($status) {
        ...
        case 'shortlisted': return 'bg-secondary';
        case 'interviewed': return 'bg-dark';
...
function getStatusIcon($status) {
    switch ($status) {
        ...
        case 'interviewed': return 'handshake';
