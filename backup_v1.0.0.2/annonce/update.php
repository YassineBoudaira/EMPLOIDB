...
if (!$id || !Security::validateInt($id)) {
    Security::redirect('../annonce/index.php', 'Invalid job ID', 'error');
}

// Get annonce data securely
$dataa = $db->fetch("SELECT * FROM annonces WHERE id = ?", [$id]);

// Check if annonce exists
if (!$dataa) {
    Security::redirect('../annonce/index.php', 'Job not found', 'error');
}
        ...
        if (empty($titre) || empty($description) || empty($email) || empty($entreprise)) {
            ...
            if (!Security::validateEmail($email)) {
                ...
                if (!$profile_id || !$contrat_id || !$ville_id || !$domaine_id || 
                    ...
                    try {
                        $image = $dataa['image']; // Keep existing image by default
                        ...
                        if (!isset($error)) {
                            $db->update("UPDATE annonces SET titre = ?, date_a = ?, image = ?, description = ?, telephone = ?, email = ?, entreprise = ?, entreprise_detaile = ?, siteweb = ?, date_fin = ?, profile_id = ?, contrat_id = ?, ville_id = ?, domaine_id = ? WHERE id = ?", 
                                       [$titre, $date_a, $image, $description, $telephone, $email, $entreprise, $entreprise_detaile, $siteweb, $date_fin, $profile_id, $contrat_id, $ville_id, $domaine_id, $id]);
                            
                            Security::redirect('../annonce/index.php', 'Job updated successfully', 'success');
                        }
                    } catch (Exception $e) {
                ...
                <div class="card-body">
                    ...
                    <form method="post" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <input value="<?= htmlspecialchars($dataa['titre']) ?>" type="text" name="titre" id="titre" class="form-control" placeholder="Titre du poste" required>
                        </div>
                        
                        <div class="form-group">
                            <label for="date_a">Date de publication</label>
                            <input value="<?= htmlspecialchars($dataa['date_a']) ?>" type="date" name="date_a" id="date_a" class="form-control" required>
                        </div>
                        
                        <div class="form-group">
                            <label for="image">Image (optionnel)</label>
                            <input type="file" name="image" id="image" class="form-control" accept="image/*">
                            <small class="form-text text-muted">Image actuelle: <?= htmlspecialchars($dataa['image']) ?></small>
                        </div>
                        
                        <div class="form-group">
                            <label for="description">Description</label>
                            <textarea name="description" id="description" class="form-control" rows="5" placeholder="Description du poste" required><?= htmlspecialchars($dataa['description']) ?></textarea>
                        </div>
                        
                        <div class="form-group">
                            <label for="telephone">Téléphone</label>
                            <input value="<?= htmlspecialchars($dataa['telephone']) ?>" type="text" name="telephone" id="telephone" class="form-control" placeholder="Téléphone">
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input value="<?= htmlspecialchars($dataa['entreprise']) ?>" type="text" name="entreprise" id="entreprise" class="form-control" placeholder="Nom de l'entreprise" required>
                        </div>
                        
                        <div class="form-group">
                            <label for="entreprise_detaile">Détails de l'entreprise</label>
                            <textarea name="entreprise_detaile" id="entreprise_detaile" class="form-control" rows="3" placeholder="Détails de l'entreprise"><?= htmlspecialchars($dataa['entreprise_detaile']) ?></textarea>
                        </div>
                        
                        <div class="form-group">
                            <label for="siteweb">Site web</label>
                            <input value="<?= htmlspecialchars($dataa['siteweb']) ?>" type="url" name="siteweb" id="siteweb" class="form-control" placeholder="Site web">
                        </div>
                        
                        <div class="form-group">
                            <label for="date_fin">Date de fin</label>
                            <input value="<?= htmlspecialchars($dataa['date_fin']) ?>" type="date" name="date_fin" id="date_fin" class="form-control" required>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="profile_id" id="profile_id" class="form-control" required>
                                ...
                                <?php foreach($profiles as $profile): ?>
                                    <option value="<?= htmlspecialchars($profile['id']) ?>" <?= $dataa['profile_id'] == $profile['id'] ? 'selected' : '' ?>>
                        ...
                        <div class="form-group">
                            ...
                            <select name="contrat_id" id="contrat_id" class="form-control" required>
                                ...
                                <?php foreach($contrats as $contrat): ?>
                                    <option value="<?= htmlspecialchars($contrat['id']) ?>" <?= $dataa['contrat_id'] == $contrat['id'] ? 'selected' : '' ?>>
                        ...
                        <div class="form-group">
                            ...
                            <select name="ville_id" id="ville_id" class="form-control" required>
                                ...
                                <?php foreach($villes as $ville): ?>
                                    <option value="<?= htmlspecialchars($ville['id']) ?>" <?= $dataa['ville_id'] == $ville['id'] ? 'selected' : '' ?>>
                        ...
                        <div class="form-group">
                            ...
                            <select name="domaine_id" id="domaine_id" class="form-control" required>
                                ...
                                <?php foreach($domaines as $domaine): ?>
                                    <option value="<?= htmlspecialchars($domaine['id']) ?>" <?= $dataa['domaine_id'] == $domaine['id'] ? 'selected' : '' ?>>
