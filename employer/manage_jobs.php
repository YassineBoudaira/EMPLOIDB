...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if ($job_id && $new_status) {
            ...
            if ($job) {
                $db->query("UPDATE annonces SET status = ? WHERE id = ?", [$new_status, $job_id]);
                $success = true;
                $success_message = 'Statut de l\'offre mis à jour avec succès !';
            } else {
...
$where_conditions = ["a.profile_id = ?"];
$params = [$employer_profile['id']];
...
<head>
    ...
    <title>Gérer mes Offres - <?= htmlspecialchars($employer['company_name']) ?> | EMPLOIDB</title>
    <!-- Favicon -->
    ...
    <div class="employer-wrapper">
        ...
        <!-- Jobs Content -->\n            <div class="container-fluid section-spacing">\n                <div class="jobs-content">
            ...
            <div class="stats-grid">
                ...
                <div class="stat-card">
                    <div class="stat-number"><?= count(array_filter($jobs, fn($job) => $job['status'] === 'active')) ?></div>
                    <div class="stat-label">Offres actives</div>
                </div>
                <div class="stat-card">
                    <div class="stat-number"><?= array_sum(array_column($jobs, 'applications_count')) ?></div>
                    <div class="stat-label">Total candidatures</div>
                </div>
                <div class="stat-card">
                    <div class="stat-number"><?= count(array_filter($jobs, fn($job) => $job['urgent'] == 1)) ?></div>
                            ...
                            <div class="col-md-4">
                                <div class="form-group element-spacing">
                                    ...
                                    <select class="form-select" id="status" name="status">
                                        ...
                                        <option value="closed" <?= $status_filter === 'closed' ? 'selected' : '' ?>>Fermée</option>
                                        <option value="expired" <?= $status_filter === 'expired' ? 'selected' : '' ?>>Expirée</option>
                                    </select>
                                </div>
                            </div>
                            <div class="col-md-4">
            ...
            <div class="jobs-card">
                <div class="card-header">
                    <h3 class="mb-0">
                        ...
                        Mes offres d'emploi (<?= count($jobs) ?>)
                    </h3>
                </div>
                            ...
                            <div class="job-card">
                                <div class="job-header">
                                    <div class="flex-grow-1">
                                        <h4 class="job-title"><?= htmlspecialchars($job['titre']) ?></h4>
                                        <div class="job-meta">
                                            <div class="job-meta-item">
                                                <i class="fas fa-map-marker-alt"></i>
                                                <?= htmlspecialchars($job['ville_nom'] ?? 'Non spécifiée') ?>
                                            </div>
                                            <div class="job-meta-item">
                                                <i class="fas fa-briefcase"></i>
                                                <?= htmlspecialchars($job['domaine_nom'] ?? 'Non spécifié') ?>
                                            </div>
                                            <div class="job-meta-item">
                                ...
                                <div class="job-actions">
                                    <a href="edit_job.php?id=<?= $job['id'] ?>" class="btn btn-outline-primary btn-sm">
                                        <i class="fas fa-edit me-1"></i>
                                        Modifier
                                    </a>
                                    <a href="applications.php?job_id=<?= $job['id'] ?>" class="btn btn-success btn-sm">
                                    ...
                                    <form method="POST" action="" style="display: inline;">
                                        ...
                                        <select name="new_status" class="form-select form-select-sm" style="width: auto; display: inline-block;" onchange="this.form.submit()">
                                            ...
                                            <option value="expired" <?= $job['status'] === 'expired' ? 'selected' : '' ?>>Expirée</option>
                                        </select>
                                    </form>
                                    
                                    <a href="../frontoffice/job_details.php?id=<?= $job['id'] ?>" target="_blank" class="btn btn-outline-primary btn-sm">
